...
Master URI:
```https://api.bagelstudio.co/api/public```
Authorization: Added as authorization headers
...
- Endpoint:
  `
  /collection/<<collectionID>>/items/<<itemID>>/field/<<fieldSlug>>
  `
    - Query Params
...
- Endpoint:
`
    /collection/<<collectionID>>/items/<<itemID>>/image?imageSlug=<fieldSlug>
`
